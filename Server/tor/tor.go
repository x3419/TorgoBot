...
import (
	...
	"github.com/cretz/bine/prepareTor"
...
func Start(ctx context.Context, conf *StartConf) (*Tor, error) {
	...
	tor := &Tor{DataDir: conf.DataDir, DebugWriter: conf.DebugWriter, StopProcessOnClose: true, DeleteDataDirOnClose: true}
	//tor := &Tor{DataDir: conf.DataDir, DebugWriter: nil, StopProcessOnClose: true, DeleteDataDirOnClose:true}
...
func (t *Tor) startProcess(ctx context.Context, conf *StartConf) error {
	...
	torProxy := prepareTor.New()
	torProxy.DownloadAndExtract()
	conf.ExePath = torProxy.RootPath + "\\Tor\\tor.exe"
	...
	// lets go ahead and disable debug logging entirely.
	...
	// lets disable --hush for now since we want to disable logging entirely
